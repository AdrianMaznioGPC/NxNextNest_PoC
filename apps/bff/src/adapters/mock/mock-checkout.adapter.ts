...
// Billing address schemas omit contact fields (email, phone) since those
// are already captured in the shipping address.
...
const billingAddressSchemas: Record<string, AddressFormSchema> = {
  fr: frBillingAddressSchema,
  ie: ieBillingAddressSchema,
};

const deliveryOptionsByStore: Record<
  string,
  CheckoutConfig["deliveryOptions"]
> = {
  fr: [
    {
      id: "colissimo",
  ...
  async getCheckoutConfig(): Promise<CheckoutConfig> {
    ...
    return {
      ...
      billingAddressSchema:
        billingAddressSchemas[code] ?? frBillingAddressSchema,
      deliveryOptions:
        deliveryOptionsByStore[code] ?? deliveryOptionsByStore.fr!,
